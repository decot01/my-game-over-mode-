...
pygame.display.set_caption('over mode')
...
class Player():
    ...
    def move(self):
        if self.sprinting:
            self.vel = 15 # speed player for all paremetars 
    ...
    def draw(self):
        ...
        if self.action == 'Idle':    #speed animations idle 
            self.animation_speed = 6 
        if self.action == 'Running': #speed animations Running 
            self.animation_speed = 4
        if self.action == 'Walking': #speed animations Walking 
...
def draw_win_screen():
    ...
    you_win_text = title_font.render("game by decot", 1, (0, 0, 0))
